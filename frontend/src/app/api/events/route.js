...
/**
 * Ritorna la lista di eventi filtrati.
 * Query params (GET):
 * - lang: "IT" | "EN" (default IT) -> serve per full-text su campi corretti
 * - q: testo libero (ilike su event/description)
 * - continent, country, location: filtri esatti
 * - group: match su group_event_en OR group_event_it
 * - year_start, year_end: filtro periodo (incrocia year_from/year_to/event_year/exact_date)
 * - limit: massimo eventi (default 2000)
 *
 * Output: array di eventi (NO wrapper).
 */
...
export async function GET(req) {
  try {
    ...
    // Colonne complete necessarie per la mappa e i pannelli
    qs.set("select", [
      "id",
      "title",
      "event_en","event_it",
    ...
    // Full-text semplice su campi localizzati
    if (q) {
    ...
    // Filtro periodo lato server (incrociato)
    if (ys != null || ye != null) rows = rows.filter(r => eventInRange(r, ys, ye));

    // Normalizzazione i18n e coordinate
    const norm = rows.slice(0, limit).map(r => normalizeI18n(r, lang));
