...
// considera privilegiati tutti i codici che iniziano per ADMIN o MOD (copre varianti)
...
export default function SettingsPage() {
  ...
  // ---- home dinamica come TopBar (fallback su FAN) ----
  const [homeHref, setHomeHref] = useState<string | null>(null);
  const [loadingHome, setLoadingHome] = useState<boolean>(true);

  // label localizzata per persona (se lingua IT mostra name_it → “Amministratore”)
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // 3) persona corrente (query separata, robusta a join mancanti)
      ...
      if (user && prof?.persona_id) {
        ...
        if (perr2) {
          // non blocchiamo la pagina, ma segnaliamo
          if (alive) setError(prev => prev ? prev : perr2.message);
        } else if (alive && pRow) {
      ...
      // 4) tutte le personas (se RLS nasconde la corrente, la inietto in lista)
      ...
      // 5) home dinamica (fallback su /landing/FAN)
      try {
        setLoadingHome(true);
  ...
  // ruolo corrente privilegiato?
  const isPrivileged = isPrivilegedCode(currentPersona?.code);

  // opzioni select:
  // - Se privilegiato, mostra SOLO la propria persona (così la vedi e non puoi cambiarla)
  // - Altrimenti mostra tutte tranne ADMIN/MOD
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    try {
      ...
      if (isPrivilegedCode(targetCode)) {
  ...
  const selectValue =
    isPrivileged
      ? (currentPersona?.id || '')    // ADMIN/MOD: forzo la propria persona e blocco
      : (profile?.persona_id || '');  // altri ruoli: selezione libera
