...
type DbWidget = {
  id: string; key?: string | null; title: string;
...
export default function StudPrimaryLanding() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!session?.user) {
          const { data: sub } = supabase.auth.onAuthStateChange(async (_e, s) => { if (s?.user) await resolve(s.user.id); });
          unsub = sub.subscription.unsubscribe; return;
        }
        await resolve(session.user.id);
      } catch (e: any) { setErrMsg(e?.message ?? "Unable to load user session."); }
    })();
  ...
  const loadWidgets = async () => {
    try {
      ...
      const { data, error } = await supabase.from("widget_personas")
      ...
      list.sort((a, b) => (a.sort_order ?? 9999) - (b.sort_order ?? 9999) || (a.title || "").localeCompare(b.title || ""));
      setWidgets(list);
    } catch (e: any) { setErrMsg(e?.message ?? "Unable to load widgets."); setWidgets([]); }
  ...
  return (
    <>
      ...
      <main className="mx-auto max-w-7xl px-4 py-10">
        ...
        {emptyState && (
          <div className="rounded-2xl border border-slate-2 00 bg-white p-8 text-center text-slate-600">
