...
/** --------------------------------------------------------------------
 *  TIPI (coerenti con il DB attuale)
 *  ------------------------------------------------------------------*/
type DbWidget = {
  id: string;                 // uuid
  key?: string | null;
...
type WidgetPersonaRow = {
  widgets: DbWidget | null;   // embedded via FK
};

/** --------------------------------------------------------------------
 *  COMPONENT
 *  ------------------------------------------------------------------*/
export default function StudHighLanding() {
  const supabase = createClientComponentClient();

  const [username, setUsername] = useState<string>("Student");
  const [personaId, setPersonaId] = useState<string | null>(null); // <-- UUID string
  const [widgets, setWidgets] = useState<DbWidget[]>([]);
  const [loading, setLoading] = useState<boolean>(true);
  const [errMsg, setErrMsg] = useState<string | null>(null);

  /** ---------------------------------------------------------------
   *  1) Carico profilo + persona (UUID, niente Number(...))
   * --------------------------------------------------------------*/
  useEffect(() => {
      ...
      try {
        ...
        if (!session?.user) {
          const { data: sub } = supabase.auth.onAuthStateChange(async (_evt, s) => {
            if (s?.user) {
              await resolveUserAndPersona(s.user.id);
            }
          });
  ...
  const loadWidgets = async () => {
    try {
      ...
      const { data, error } = await supabase
        ...
        .eq("persona_id", personaId) // <-- UUID string
      ...
      // Ordino usando widgets.sort_order (se presente), poi title
      list.sort((a, b) => {
  ...
  // ricarico quando ottengo personaId
  useEffect(() => {
    if (personaId) {
      loadWidgets();
    }
        ...
        {emptyState && (
          <div className="rounded-2xl border border-slate-200 bg-white p-8 text-center text-slate-600">
            ...
            <p className="mt-1 text-sm">
              Ask the admin to link widgets to this persona in <code>widget_personas</code>.
            </p>
          </div>
        )}
